 ...
 *
 */
public class Tap3XMMdr7  extends RootClass {
	private static final String baseDir = System.getProperty("java.io.tmpdir") ;
	private static final String baseUrlN = "http://xcatdb.unistra.fr/3xmmdr7/tap/";
	...
	public static void main(String[] args) throws Exception {
		...
		for(Object sn: schemas) {
			...
			if( ((String)(s.get("name"))).equals("EPIC") ) {
				JSONArray tables = (JSONArray) s.get("tables");
				for( Object ts: tables) {
					JSONObject t = (JSONObject)ts;
					if( ((String)(t.get("name"))).endsWith("EPIC_SPECTRUM") ) {
						System.out.println("found " + s.get("name").toString());
